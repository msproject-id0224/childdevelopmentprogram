...
export default function CommunicationIndex({ auth }) {
    ...
    const fetchOnlineUsers = async (query = '') => {
        try {
            const response = await axios.get(`/api/online-users${query ? `?query=${query}` : ''}`);
    ...
    // Typing Indicator Listener (Mocking real-time if Echo is not fully set up or using fallback)
    useEffect(() => {
                                ...
                                {onlineUsers.map(u => (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{u.last_active}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                                        ...
                                        {filteredMessages.map(msg => (
                                            <div key={msg.id} className={`flex flex-col ${Number(msg.sender_id) === Number(user.id) ? 'items-end' : 'items-start'}`}>
                                                    ...
                                                    {msg.attachment_path && (
                                                        <div className="mb-2">
                                                            {msg.attachment_type?.startsWith('image/') ? (
                                                                <img src={msg.attachment_path.startsWith('blob:') ? msg.attachment_path : `/storage/${msg.attachment_path}`} alt={__('Attachment')} className="max-w-full rounded-lg" />
                                                            ) : (
                                                ...
                                                <div className="flex items-center gap-1 mt-1">
                                                    <span className="text-[10px] text-gray-500">{new Date(msg.created_at).toLocaleTimeString(locale === 'id' ? 'id-ID' : 'en-US')}</span>
                                                    {Number(msg.sender_id) === Number(user.id) && (
